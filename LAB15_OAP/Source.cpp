...
//-------------------------------
int Object::Multiplicative(int key, int size, int p, char* arr) {
	int const c = 0.618;
	return floor(size*(key*c-floor(key*c)));//3 varik
}
//-------------------------------
int Object::Modul(int key, int size, int p, char* arr) {
	return key % size;//9varik
}

int Object::universal(int key, int size, int p, char* arr) {
	int h = 0, a = 314445, b = 37542;
	h = (a * h + key) % size;
	return (h<0)?(h+size):h;//7varik
}
